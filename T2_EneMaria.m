...
P_es = 0.05; % perioada de esantionare
vtimp=0:P_es:D; % vectorul de timp pentru care semnalul este dreptunghi
x_tr= sawtooth((2*pi/D)*vtimp,0.5)/2+0.5; % generarea semnalului triunghiular
t = 0:P_es:P-P_es; % timpul total al semnalului

x = zeros(1,length(t)); % x format numai din 0
x(t<=D)=x_tr; % adaugarea partii triunghiulare peste semnal

x_plot = repmat(x,1,4); % repetarea pe 4 perioade

t_plot = 0:P_es:4*P-P_es;  % timpul pe 4 perioade
figure();
plot(t_plot,x_plot); % afisare grafic
...
for i = 1: length(t)
    x_2(i) = 0;
    for k=-50:50
        x_2(i) = x_2(i) + (1/P)*X(k+51)*exp(j*k*w0*t(i)); % se calculeaza semnalul in punctul i
    end
end
